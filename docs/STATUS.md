...
- 优先做 V1.1.1 上下文可消费性补强：context schema/capability 信息、MCP 重启提示、目标画板聚焦、下载图片真实尺寸和缩放比例。
- 将组件级还原纳入 V1.1.1 设计：支持目标区域或目标描述，避免只能整页还原。
- 随后推进 V1.2 单图详情增强，优先寻找图层、文本、标注和切图入口。
...
- Codex MCP 长进程可能在代码更新后继续使用旧构建；后续需要 schema/capability 自检和重启提示。
